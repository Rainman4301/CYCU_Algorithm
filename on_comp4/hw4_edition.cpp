...
void out_influence_file(map<string,trunk> &main_chain,string filename){
...
void influence(map<string,trunk> main_chain,string filename ){
    ...
    for( auto& p: influence){
        ...
        trunk *curr = p.second.point;
         trunk *back = curr->point;
        trunk *front = &p.second;
    ...
    for(const auto& i: influence){
        cout << i.first <<":"<< endl;
        for (trunk *j = i.second.point; j != NULL; j = j->point)
        {
            cout << j->ID << " " << j->weight<<endl;
        }
    }
    ...
    out_influence_file(influence,filename);
}
