...
void Quadratic_probing(vector<studenttype> &data , int count,string filename , int least){
    ...
    do{
        ...
        int key = 1;
        int sidLen = find_stid.length();
        for (int j = 0 ; j < sidLen; j++)
        {
            key = (key * find_stid[j]) % count;
        }
        ...
        {   
            ...
            searching_count++;
            // avoid infinity loop
            if(!( strcmp(quadratic_table[key + k*k].data.sid , find_stid.c_str()) ) ){

                cout << "\n{ "<< quadratic_table[key + k*k].data.sid<<" , "<< quadratic_table[key + k*k].data.sname<<" , "<< quadratic_table[key + k*k].data.mean<< " }"<< " is found after "<< searching_count <<" probes. "<< endl;
                break;
            }
            
            int index = (key + (long long int)k*k) % count;
            if( quadratic_table[index].data.mean == 0 || k == (count/2)+1 ){

                cout <<"\n"<< find_stid <<" is not found after "<< searching_count << " probes." << endl;
                break;
            }
        }
...
void Double_hashing(vector<studenttype> &data , int count,string filename , int least){
    ...
    vector<hash_node> double_table(count, hash_node());
    fstream outfile;
    ...
    for(int i = 0 ; i < data.size() ; i++){
        ...
        for (int k = 0 ; ; k++)
        {
            ...
            if( double_table[ index ].status == hash_node_status::Unused || double_table[ index ].status == hash_node_status::Deleted ){
                double_table[index].key = key;
                double_table[index].data = data[i];
                double_table[index].status = hash_node_status::Occupied;
                break;
            }
        }
    }
    ...
    for (int i = 0; i < count;i++){
        outfile<< '['<< setw(3) <<i<<"] ";
        if(double_table[i].status == hash_node_status::Occupied){
            outfile << setw(10) << double_table[i].key << ", ";
            outfile << setw(10) << double_table[i].data.sid << ", ";
            outfile << setw(10) << double_table[i].data.sname << ", ";
            outfile << toString(double_table[i].data.mean, 10);
        }
        outfile<<"\n";
    }
    outfile << " ----------------------------------------------------- \n";
    ...
    do{
        ...
        {   
            ...
            searching_count++;
            // avoid infinity loop
            if(!( strcmp(double_table[( key+ ( k *(highest-temp) ) )%count].data.sid , find_stid.c_str()) ) ){

                cout << "\n{ "<< double_table[( key+ ( k *(highest-temp) ) )%count].data.sid<<" , "<< double_table[( key+ ( k *(highest-temp) ) )%count].data.sname<<" , "<< double_table[( key+ ( k *(highest-temp) ) )%count].data.mean<< " }"<< " is found after "<< searching_count <<" probes. "<< endl;
                break;
            }

            int index = (key + (k * (highest - temp))) % count;
            if( double_table[index].data.mean == 0 ){
