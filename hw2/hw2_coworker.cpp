...
namespace tree_util {
	...
	public:
		...
		void push(const TKey& key, const TData& data) {
			if (root == NULL) {
				...
				find_node->insert_data(i, key, data);
				while (find_node != NULL && find_node->get_data_count() >= 3) {
					...
					new_left_node->insert_data(0, find_node->data[0]);
					new_right_node->insert_data(0, find_node->data[2]);
					if (child_count >= 1) {
						new_left_node->insert_child(0, find_node->childs[0]);
					}
					if (child_count >= 2) {
						new_left_node->insert_child(1, find_node->childs[1]);
					}
					if (child_count >= 3) {
						new_right_node->insert_child(0, find_node->childs[2]);
					}
					if (child_count >= 4) {
						new_right_node->insert_child(1, find_node->childs[3]);
					}
					if (tmp_parent != NULL) {
		...
		twothree_treenode* search_node(const TKey& find_key, int& data_index) {
			...
			while (find_node != NULL) {
				...
				int data_count = find_node->get_data_count();
				for (i = 0; i < data_count; i++) {
					const TKey& cmp_obj = find_node->data[i].key;
					int cmp_result = base_comparer(cmp_obj, find_key);
					if (cmp_result > 0) {
						break;
					}
					else if (cmp_result == 0) {
		...
		TData& search(const TKey& find_key, bool& exists){
			...
			if (searched_node == NULL) {
				...
				exists = true;
				return searched_node->data[data_index].data;
			}
		}
		TData pop_key(const TKey& key) {
			int index = 0;
			twothree_treenode* find_node = search_node(key, index);
			if (find_node == NULL)
				throw runtime_error("key not exists.");
			TData output = find_node->data[index].data;
			remove_node_keypair(find_node, index);
		}
		key_pair<TKey, TData> pop_min() {
			if (root == NULL)
				throw runtime_error("2-3 tree is empty.");
			twothree_treenode* find_node = root;
			while (find_node->get_child_count() > 0) {
				find_node = find_node->childs[0];
			}
			key_pair<TKey, TData> output = find_node->data[0];
			remove_node_keypair(find_node, 0);
			return output;
		}
		key_pair<TKey, TData> pop_max() {
			if (root == NULL)
				throw runtime_error("2-3 tree is empty.");
			twothree_treenode* find_node = root;
			int child_count;
			while ((child_count = find_node->get_child_count()) > 0) {
				find_node = find_node->childs[child_count - 1];
			}
			int data_count = find_node->get_data_count();
			key_pair<TKey, TData> output = find_node->data[data_count - 1];
			remove_node_keypair(find_node, data_count - 1);
			return output;
		}
		...
		// Debug
		bool is_correct_twothree() {
			queue< twothree_treenode* > nodes;
			if (root != NULL)
				nodes.push(root);
			while (!nodes.empty()) {
				twothree_treenode* cur_node = nodes.front();
				nodes.pop();
				if (cur_node->get_child_count() > 0 && (cur_node->get_child_count() - cur_node->get_data_count()) != 1) {
					return false;
				}
				for (int i = 0; i < cur_node->get_child_count(); i++) {
					nodes.push(cur_node->childs[i]);
				}
			}
			return true;
		}
	...
	private:
		...
		// Methods
		void remove_node_keypair(twothree_treenode* node, int index) {
			twothree_treenode* find_node = node;
			if (find_node == NULL)
				return;
			find_node->remove_data(index);
			if (find_node->get_child_count() > 0) { // find_node will not be leaf.
				twothree_treenode* successor = find_node->childs[index + 1];
				while (successor->get_child_count() > 0) {
					successor = successor->childs[0];
				}
				find_node->insert_data(index, successor->data[0]);
				successor->remove_data(0);
				find_node = successor;
			}
			while (find_node != NULL && find_node->get_data_count() == 0) {
				twothree_treenode* parent_node = find_node->parent;
				if (parent_node == NULL) {
					if (find_node->get_child_count() >= 1) {
						root = find_node->childs[0];
						find_node->childs[0]->parent = NULL;
					}
					else {
						root = NULL;
					}
					...
					int i;
					for (i = 0; i < parent_node->get_child_count(); i++) {
						if (parent_node->childs[i] == find_node)
							break;
					}
					if (i > 0) {
						twothree_treenode* sibling_node = parent_node->childs[i - 1];
						find_node->insert_data(0, parent_node->data[i - 1]);
						parent_node->remove_data(i - 1);
						for (int j = 0; j < sibling_node->get_data_count(); j++) {
							find_node->insert_data(j, sibling_node->data[j]);
						}
						for (int j = 0; j < sibling_node->get_child_count(); j++) {
							find_node->insert_child(j, sibling_node->childs[j]);
						}
						parent_node->remove_child(i - 1);
						i = i - 1;
					}
					else {
						twothree_treenode* sibling_node = parent_node->childs[i + 1];
						find_node->insert_data(0, parent_node->data[i]);
						parent_node->remove_data(i);
						for (int j = 0; j < sibling_node->get_data_count(); j++) {
							find_node->insert_data(j + 1, sibling_node->data[j]);
						}
						for (int j = 0; j < sibling_node->get_child_count(); j++) {
							find_node->insert_child(j + 1, sibling_node->childs[j]);
						}
						parent_node->remove_child(i + 1);
					}
					if (find_node->get_data_count() >= 3) {
						int child_count = find_node->get_child_count();
						twothree_treenode* new_left_node = new twothree_treenode();
						twothree_treenode* new_right_node = new twothree_treenode();
						new_left_node->insert_data(0, find_node->data[0]);
						...
						parent_node->insert_data(i, find_node->data[1]);
						parent_node->remove_child(i);
						parent_node->insert_child(i, new_left_node);
						parent_node->insert_child(i + 1, new_right_node);
					}
				}
	...
	public:
		...
		binary_treenode<TKey, TData>* get_max_node() const{
			binary_treenode<TKey, TData>* find_node = root;
			if (root == NULL) {
				throw runtime_error("binary tree is empty.");
			}
			while (find_node->have_rightchild()) {
				find_node = find_node->rightchild;
			}
			return find_node;
		}

		binary_treenode<TKey, TData>* get_min_node(){
			binary_treenode<TKey, TData>* find_node = root;
			if (root == NULL) {
				throw runtime_error("binary tree is empty.");
			}
			while (find_node->have_leftchild()) {
				find_node = find_node->leftchild;
			}
			return find_node;
		}
		...
		TData pop_key(const TKey& key) {
			...
			if (find_node == NULL) {
				...
				TData output = find_node->nodedata.data;
		...
		key_pair<TKey,TData> pop_min() {
			...
			key_pair<TKey, TData> output = find_node->nodedata;
			remove_node(find_node);
			return output;
		}
		key_pair<TKey, TData> pop_max() {
	...
	protected:
		...
		TData default_value;
		// Methods
		virtual binary_treenode<TKey, TData>* push_node(binary_treenode<TKey, TData>* new_node) {
			if (root == NULL) {
				root = new_node;
			}
			else {
				const TKey& key = new_node->nodedata.key;
				binary_treenode<TKey, TData>* find_node = root;
		...
		virtual binary_treenode<TKey, TData>* remove_node(binary_treenode<TKey, TData>* del_node) {
			if (!del_node)
				return NULL;
			binary_treenode<TKey, TData>* find_node = del_node;
			binary_treenode<TKey, TData>* affected_node = NULL;
			if (del_node->have_rightchild()) {
				find_node = find_node->rightchild;
				while (find_node->have_leftchild()) {
					find_node = find_node->leftchild;
				}
				binary_treenode<TKey, TData>* find_node_parent = find_node->parent;
				binary_treenode<TKey, TData>* find_node_rightchild = find_node->rightchild;
				if (del_node->parent != NULL) {
					if (del_node->is_leftnode())
						del_node->parent->set_leftchild(find_node);
					else
						del_node->parent->set_rightchild(find_node);
				}
				else {
					root = find_node;
					find_node->parent = NULL;
				}
				find_node->set_leftchild(del_node->leftchild);
				if (find_node_parent != del_node) {
					find_node->set_rightchild(del_node->rightchild);
					find_node_parent->set_leftchild(find_node_rightchild);
					affected_node = find_node_parent;
				}
				else {
					affected_node = find_node;
				}
			}
			else if (del_node->have_leftchild()) {
				find_node = del_node->leftchild;
				if (del_node->is_leftnode())
					del_node->parent->set_leftchild(find_node);
				else if (del_node->is_rightnode())
					del_node->parent->set_rightchild(find_node);
				else {
					root = find_node;
					find_node->parent = NULL;
				...
				affected_node = del_node->parent;
				if (del_node->is_leftnode()) {
					del_node->parent->set_leftchild(NULL);
				}
				else if (del_node->is_rightnode()) {
					del_node->parent->set_rightchild(NULL);
				}
				else {
					root = NULL;
				}
	...
	public:
		...
		void print_tree() {
			vector<queue<binary_treenode<TKey, TData>*>> d_queue = vector<queue<binary_treenode<TKey, TData>*>>(2, queue<binary_treenode<TKey, TData>*>());
			int available_nodes[2] = {};
			if (this->root != NULL) {
				available_nodes[1] = 1;
			}
			d_queue[1].push(this->root);
			for (int i = 1; available_nodes[i & 1] > 0; i++) {
				available_nodes[(i & 1) ^ 1] = 0;
				queue<binary_treenode<TKey, TData>*>& read_que = d_queue[i & 1];
				queue<binary_treenode<TKey, TData>*>& write_que = d_queue[(i & 1) ^ 1];
				while (!read_que.empty()) {
					binary_treenode<TKey, TData>* top_node = read_que.front();
					read_que.pop();
					if (top_node != NULL) {
						int bf = calculate_balance_factor(top_node);
						printf("[%3d|b:%d] ", top_node->nodedata.key, bf );
						available_nodes[(i & 1) ^ 1]++;
						write_que.push(top_node->leftchild);
						write_que.push(top_node->rightchild);
					}
					else {
						printf("[e|e:0] ");
						write_que.push(NULL);
	...
	protected:
		binary_treenode<TKey, TData>* push_node(binary_treenode<TKey, TData>* new_node) {
			binary_tree<TKey, TData>::push_node(new_node);
			balance_tree(new_node);
			return new_node;
		}
		binary_treenode<TKey, TData>* remove_node(binary_treenode<TKey, TData>* del_node) {
			binary_treenode<TKey, TData>* affected_node = binary_tree<TKey, TData>::remove_node(del_node);
			if (affected_node != NULL) {
				balance_tree(affected_node);
			}
			return affected_node;
		}
	private:
		...
		binary_treenode<TKey, TData>* rotate_left(binary_treenode<TKey, TData>* node) {
			...
			if (!node->is_root()) {
				if (node->is_leftnode()) {
					node->parent->set_leftchild(old_node_left);
				}
				else {
					node->parent->set_rightchild(old_node_left);
				}
			}
			else {
		...
		void balance_tree(binary_treenode<TKey, TData>* node) {
			...
			int prev_bf = 0x80000000;
			int prev_node_rel = 2; // 0: left 1: right 2: unset
			int nodeleft_height = -1;
			int noderight_height = -1;
			while (balance_node != NULL) {
				if (nodeleft_height == -1)
				...
				if (bf == 2) {	//LL or LR
					if(prev_bf == 0x80000000 || prev_node_rel != 0)
					...
					if (prev_bf == 0x80000000 || prev_node_rel != 1)
				...
				if (balance_node->is_leftnode()) {
					nodeleft_height = max(nodeleft_height, noderight_height) + 1;
					noderight_height = -1;
					prev_node_rel = 0;
				}
				else if (balance_node->is_rightnode()) {
...
int get_total_record_count() {
	if (avltree.is_empty())
		throw runtime_error("AVL Tree is empty.");
	int count = 0;
	queue <binary_treenode<int, vector<graduation_record>>*> que;
	que.push(avltree.get_root());
	while (!que.empty()) {
		binary_treenode<int, vector<graduation_record>>* top_ele = que.front();
		que.pop();
		if (top_ele->have_leftchild()) {
			que.push(top_ele->leftchild);
		}
		if (top_ele->have_rightchild()) {
			que.push(top_ele->rightchild);
		}
		count += top_ele->nodedata.data.size();
	}
	return count;
}
void Task_1() {
	...
	for (int i = 0; i < input_table.get_row_count(); i++) {
		...
		tree[record.school_name].push_record(record);
	}
...
void Task_2() {
	...
	bool exists;
	const vector<graduation_record>& tmpp = avltree.search(13, exists);
...
void Task_3() {
	if (avltree.is_empty())
		throw runtime_error("AVL Tree is empty.");
	int k;
	bool success, eof_token;
	int records = get_total_record_count();
	do {
		printf("Enter k (0, %d]: ", records);
		k = stdioutil::read_int32(success, eof_token);
		if (k <= 0 || k > records) {
			printf("K should be in (0, %d]\n", records);
			success = false;
		}
		if (eof_token) {
			printf("User stop the operation.\n");
	...
	int counter = 1;
	binary_treenode<int, vector<graduation_record>>* max_node = avltree.get_max_node();
	while (k > 0) {
		vector<graduation_record>& records = max_node->nodedata.data;
		for (const graduation_record& record : records) {
			printf("%d: [%d] %s, %s, %s, %s, %d\n",
				counter++,
				record.record_id,
				record.school_name.c_str(),
		...
		if (max_node->have_leftchild()) {
			max_node = max_node->leftchild;
			while (max_node->have_rightchild())
				max_node = max_node->rightchild;
		}
		else {
			binary_treenode<int, vector<graduation_record>>* prev_node = max_node;
			max_node = max_node->parent;
			while (max_node != NULL && max_node->leftchild == prev_node) {
				prev_node = max_node;
				max_node = max_node->parent;
			}
		}
...
void Task_4() {
	twothree_tree<int, int> tree = twothree_tree<int, int>(int_compare);
	avl_tree<int, int> avl = avl_tree<int, int>(int_compare);
	for (int i = 0; i < 999; i++) {
		int num = (i * 1709 + 12539) % 999;
		tree.push(num, num);
		avl.push(num, num);
	}
	while (!tree.is_empty()) {
		key_pair<int,int> min_A = tree.pop_min();
		key_pair<int, int> min_B = avl.pop_min();
		bool is_correct = tree.is_correct_twothree();
		bool is_balance = avl.check_avltree();
		printf("min: 2-3<%3d %5s)> avl<%3d %5s>\n", min_A.key, is_correct ? "true" : "false", min_B.key, is_balance ? "true" : "false");
		avl.print_tree();
	}
}
