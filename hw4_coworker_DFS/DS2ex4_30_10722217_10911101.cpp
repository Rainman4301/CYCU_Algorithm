...
namespace studentGraph {
	...
	public:
		...
		int getConnCount(const string& senderID, vector<vertexInfo*>& output);
		int getConnCount(vertexInfo* const senderVertex, vector<vertexInfo*>& output);
	...
	void digraph::writeConnResultToFile(const string& fileName) {
		...
		unordered_map <vertexInfo*, vector<vertexInfo*>> output;
		vector <pair<int, vertexInfo*>> connCounter;
		for (const auto& v : vertices) {
			int innerIndex = 1;
			vertexInfo* vertexInf = v.second;
			output[v.second] = vector<vertexInfo*>();
			vector<vertexInfo*>& connVertices = output[v.second];
			int connCount = getConnCount(vertexInf, connVertices);
		...
		for (const auto& connCountInf : connCounter) {
			vector<vertexInfo*>& connVertices = output[connCountInf.second];
			int innerIndex = 1;
	...
	int digraph::getConnCount(const string& senderID, vector<vertexInfo*>& output) {
		const auto& senderVertexIter = vertices.find(senderID);
		if (senderVertexIter == vertices.end())
			return -1;
		vertexInfo* const senderVertex = (*(senderVertexIter)).second;
		getConnCount(senderVertex, output);
	}
	int digraph::getConnCount(vertexInfo* const senderVertex, vector<vertexInfo*>& output) {
		...
		sort(output.begin(), output.end(), vertexInfoPtrComparer());
		return output.size();
	}
...
void printUsage() {
	...
	if (_programStatus.haveOpenFile)
		...
		printf("\t\033[31m2) Create double hash.\033[0m\n");
	printf("\t0) Exit.\n");
