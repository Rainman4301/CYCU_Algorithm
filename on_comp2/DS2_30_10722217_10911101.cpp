...
void top_value( int k , avl_ptr root , vector<avl_ptr> &track){

    avl_ptr temp = root;

    if(temp){
        top_value(k, temp->lef_ptr,track);
        track.push_back(root);
        top_value(k, temp->rig_ptr,track);
    }
...
int main()
{
    ...
    cout <<"* 3. Top-K search on AVL tree (please input case 2 first)*"<< endl;
    cout << "*************************************" << endl;
    ...
    while (option != 0)
    {
        ...
        switch (option)
        {
            ...
            case 2:
                ...
                cout << "Enter K in [1," << sugardady.size() - 1 << "]: ";
                cin >> k;
                cout<<"\n\n"<<endl;

                top_value(k, root2, track);

                for (int i = track.size() - 1, q = k , count = 1   ; i > 0, q > 0;      i--, q--)
                {
                    for (int j = 0 ; j <= track[i]->key.size() - 1; j++, count++)
                    {
                        cout <<count << " : "<< " [" << sugardady[track[i]->key[j].num].data << "] " << sugardady[track[i]->key[j].num].school_name <<","<< sugardady[track[i]->key[j].num].major_name <<","<< sugardady[track[i]->key[j].num].nighday<<"," << sugardady[track[i]->key[j].num].level<<"," << sugardady[track[i]->key[j].num].ngrade << endl;
                    }
                    if(count > k){
                        break;
                    }
